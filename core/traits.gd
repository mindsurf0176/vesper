...
## 원소 시너지. 플레이어가 기억할 규칙은 한 줄이다 —
## **같은 원소를 모으면 세진다. 셋을 다 모으면 트라인.**
##
## 역할(근접·원거리·방어·지원)은 상성만 담당하고 시너지에는 관여하지 않는다.
## 둘 다 시너지를 주면 효과가 겹쳐서 규칙만 늘고 이해할 것이 두 배가 된다.
##
## 오토체스 관례대로 **고유 별자리 수**로 센다(같은 별 셋은 1로 침).
## 원소당 별자리가 셋뿐이므로 최대 단계가 3이고, 그게 곧 트라인이다.
...
var element_counts: Dictionary = {}  ## Element -> 고유 별자리 수
...
## placements: [{def_id, star, order}] — 강림 큐에 올라간 별만 넘길 것.
...
## 별 하나에 적용될 최종 보정치. sim이 생성 시 한 번 호출한다.
