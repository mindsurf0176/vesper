...
- 현재 빌드: **원정 브리핑 → 5인 스쿼드 선택 → 회랑 분기 → 실시간 출격 카드 전투 → 유물/휴식 → 5층 보스**가 실제 실행된다. 이전 8인 매치와 메타 화면은 레거시 보존용으로 비활성화되어 있다.
...
- **5인 고정 원정대 + 실시간 출격 카드**: 전투 중 하단 카드로 코스트가 찬 사도를 직접 출격한다.
...
- **이번 제출 범위**: 현재 런 안에서만 유지되는 5인 원정대와 유물 성장. 상점·가챠·계정 메타는 비활성 레거시다.
...
- 첫 전투 전에 원정대 5명을 고르고 `전투 시작`을 누른다.
