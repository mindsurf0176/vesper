...
- 현재 빌드: **원정 브리핑 → 6명 중 4인 스쿼드 선택 → 회랑 분기 → 실시간 출격 카드 전투 → 유물/휴식 → 5층 보스**가 실제 실행된다. 이전 8인 매치와 메타 화면은 레거시 보존용으로 비활성화되어 있다.
...
- **4인 고정 원정대 + 실시간 출격 카드**: 6명 중 4명을 고른 뒤 전투 중 하단 카드로 코스트가 찬 사도를 직접 출격한다.
...
- 첫 전투 전에 6명 중 원정대 4명을 고르고 `전투 시작`을 누른다.
