...
func _test_event_mapping() -> void:
	...
	sim._events.append({"t": "combo", "name": "엄호 사격", "uid": ally.uid})
	presenter._process(Defs.TICK)
	ok(String(presenter._ability_calls.back().get("label", "")).contains("출격 연계"), "출격 연계가 전투 배너로 표시된다")
	sim._events.append({"t": "break", "name": "전선 돌파", "cost": 2.0})
	presenter._process(Defs.TICK)
	ok(String(presenter._ability_calls.back().get("label", "")).contains("전선 돌파"), "전선 돌파 보너스가 전투 배너로 표시된다")
